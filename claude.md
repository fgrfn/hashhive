...
**Gerätekonfiguration-Felder** (per Gerät, gesendet an `POST /config?ip=`):
```json
{
  "ip": "10.10.40.112",
...
| PEP 668 bei `pip install` | Externally managed environment | setup.sh nutzt `.venv/` Virtualenv |
| `sudo: command not found` in setup.sh | Script läuft als root | `SUDO`-Variable wird per `id -u` gesetzt |
| NMMiner-Konfig kann nicht gespeichert werden | Endpunkt-Variante unbekannt | POST-Fallback-Kette: master?ip → master → direkt |
